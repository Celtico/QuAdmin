...
/**
 * @property mixed _referencesByObject
 */
class ObjectCache
{
    /**
...
//Profiler::start('OBJECT_SAVE');
        ...
        if (isset($this->_objects[$idx])) {
            throw new \Exception('Object already exists in registry ('.$idx.'). Old object class: '.get_class($this->_objects[$idx]).', new object class: '.get_class($object));
        }
...
//Profiler::stop('OBJECT_SAVE');
    ...
    {
//Profiler::start("OBJECT_DELETE");
        if (is_object($idx)) {
            $idx = $this->find($idx);
            if (false===$idx) {
//Profiler::stop("OBJECT_DELETE");
                return false;
            }
...
//Profiler::stop("OBJECT_DELETE");
        ...
        if (isset($this->_objectReferences[$idx])) {
            foreach ($this->_objectReferences[$idx] as $r=>$dummy) {
